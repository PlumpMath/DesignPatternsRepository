...
namespace Xrm.ReportUtility
{
    public static class Program
    {
        ...
        {
            var service = GetReportService(args);       //тут демонстрация фабричного метода
        ...
        private static IReportService GetReportService(string[] args)
        {
            ...
            if (filename.EndsWith(".txt"))              //для цепочки if лучше использовать Цепочку ответственности
            {
        ...
        private static void PrintReport(Report report)
        {
            if (report.Config.WithData && report.Data != null && report.Data.Any())
            {
                ...
                if (report.Config.WithIndex)                        //тоже самое: для цепочки if лучше использовать Цепочку ответственности
                {
                    headerRow = "№\t" + headerRow;
